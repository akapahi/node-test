...
var af = 60;
var state = 0;
let boid_p_slider, boid_p;
let boid_a_slider, boid_a;
let boid_s_slider, boid_s;
let boid_c_slider, boid_c;
let boid_f_slider, boid_f;
let boid_ms_slider, boid_ms;
let boid_ds_slider, boid_ds;
...
function setup() {
	...
	loadJSON('/params', (params)=>{
		var keys = Object.keys(params);
		boid_p = params[keys[0]];
		boid_s= params[keys[1]];
		boid_a= params[keys[2]];
		boid_c= params[keys[3]];
		boid_f= params[keys[4]];
		boid_ds= params[keys[5]];
		boid_ms= params[keys[6]];	
		...
		boid_p_slider = createSlider(0, 500, boid_p, 10);
		boid_s_slider = createSlider(0, 5, boid_s, 0.5);
		boid_a_slider = createSlider(0, 5, boid_a, 0.5);
		boid_c_slider = createSlider(0, 5, boid_c, 0.5);
		boid_f_slider = createSlider(0, 5, boid_f, 0.5);
		boid_ds_slider = createSlider(0, 100, boid_ds, 5);
		boid_ms_slider = createSlider(1, 5, boid_ms, 0.5);
...
function updateValues() {
	const boid_p = boid_p_slider.value();
	const boid_s = boid_s_slider.value();
	const boid_a = boid_a_slider.value();
	const boid_c = boid_c_slider.value();
	const boid_f = boid_f_slider.value();
	const boid_ds = boid_ds_slider.value();
	const boid_ms = boid_ms_slider.value();

	for (let boid of flock) {
		boid.perception = boid_p;
		boid.separationForce = boid_s;
		boid.alignmentForce = boid_a;
		boid.cohesionForce = boid_c;
		boid.flightForce = boid_f;
		boid.desiredSeparation = boid_ds;
		boid.maxSpeed = boid_ms;
	}
